...
if (!mobileMenu || !openBtn || !closeBtn) return;openBtn.addEventListener('click', () => {
...
const mult = getMultiplier();for (let slot = 0; slot < 5; slot++) {
...
const isCenter = slot === 2;card.style.zIndex = cfg.z;
...
}card.style.transform = `translateX(${tx}px) translateY(${ty}px) rotate(${rot}deg) scale(${cfg.scale})`;if (isCenter) {
